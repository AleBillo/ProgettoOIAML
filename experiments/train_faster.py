...
class Trainer:
    def __init__(self, model, train_loader, test_loader=None, device=None, lr=0.001):
        ...
        self.device = device or torch.device(
            "cuda"
            if hasattr(torch, "cuda") and torch.mps.is_available()
...
def main():
    ...
    train_dataset = RPSDataset(
        root_dir=train_dir, transform=Augmentations.get_train_transforms()
    )

    test_dataset = RPSDataset(
        root_dir=test_dir, transform=Augmentations.get_test_transforms()
    )
    ...
    trainer.save_model("model_faster.pth")
